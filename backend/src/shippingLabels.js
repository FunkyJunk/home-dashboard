...
const ANALYZE_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      labelDescription: {
        type: "string",
        description: "Describe what you actually see, in this order: (1) where the label's own bordered 'main shipping info' box is relative to the rest of the image - the printed rectangle enclosing the postage stamp/logo and the ship-from/ship-to addresses; (2) the label's full internal layout top-to-bottom, including whatever sits below that bordered box - typically a barcode, tracking number text, and a QR code; (3) two independent checks on orientation, since address text can be small or blurry in a photo - first, which image-edge the postage stamp/logo block is nearest to versus which image-edge the barcode/tracking-number block is nearest to (these are always at opposite ends of the label and are usually the easiest large elements to place); second, look at the address text inside the ship-to block and determine which way it currently reads - normally upright, sideways (rotated 90 either way), or upside down. If these two checks disagree, trust the postage-vs-barcode placement, since it does not depend on reading small text. Base every field below on this description.",
      },
      marketplace: {
        type: ["string", "null"],
